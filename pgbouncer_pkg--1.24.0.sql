 ...
 * PgBouncer 24 
 * new fields:
 *   replication(ACTIVE_SOCKETS)
 *   replication(CLIENTS)
 *   id(CLIENTS) 
 *   server_lifetime(DATABASES)
 *   load_balance_hosts (DATABASES)
 *   max_client_connections (DATABASES)
 *   current client_connections (DATABASES)
 *   load_balance_hosts(POOLS)
 *   replication (SERVERS)
 *   id (SERVERS)
 *   replication (SOCKETS)
 *   id (SOCKETS)
...
CREATE SERVER IF NOT EXISTS pgbouncer FOREIGN DATA WRAPPER dblink_fdw OPTIONS (host 'localhost', port '6432', dbname 'pgbouncer');
...
COMMENT ON COLUMN pgbouncer.active_sockets."replication" IS $$TO DO...$$;
...
COMMENT ON COLUMN pgbouncer.active_sockets."prepared_statements" IS $$TO DO...$$;
COMMENT ON COLUMN pgbouncer.active_sockets."id" IS $$TO DO...$$;
...
COMMENT ON COLUMN pgbouncer.clients."replication" IS $$TO DO...$$;
...
COMMENT ON COLUMN pgbouncer.clients."prepared_statements" IS $$TO DO...$$;
COMMENT ON COLUMN pgbouncer.clients."application_name" IS $$A string containing the application_name set on the linked client connection, or empty if this is not set, or if there is no linked connection.$$;
...
CREATE OR REPLACE VIEW pgbouncer.databases AS
    SELECT * FROM dblink('pgbouncer', 'show databases') AS _(
        ...
        server_lifetime integer, --v24
        pool_mode text,
        load_balance_hosts integer, --v24
        max_connections integer,
        current_connections integer,
        max_client_connections integer, --v24
        current_client_connections integer, --v24
        paused integer,
        disabled integer
    );
...
CREATE OR REPLACE VIEW pgbouncer.stats AS
    SELECT * FROM dblink('pgbouncer', 'show stats') AS _(
        database text,
        total_server_assignment_count integer, --v24
        ...
        total_client_parse_count bigint, --v24
        total_server_parse_count bigint, --v24
        total_bind_count bigint, --v24
        avg_server_assignment_count bigint, --v24
        ...
        avg_client_parse_count bigint, --v24
...
CREATE OR REPLACE VIEW pgbouncer.stats_totals AS
    SELECT * FROM dblink('pgbouncer', 'show stats_totals') AS _(
        ...
        query_time bigint,
        wait_time bigint,
        client_parse_count bigint, --v24
				server_parse_count bigint, --v24
				bind_count         bigint --v24
    );
...
CREATE OR REPLACE VIEW pgbouncer.users AS
    SELECT * FROM dblink('pgbouncer', 'show users') AS _(
        name text,
        pool_size bigint, --v24
        reserve_pool_size bigint, --v24
        pool_mode text,
        max_user_connections integer, --v24
        current_connections  integer, --v24
        max_user_client_connections integer, --v24
        current_client_connections integer --v24
    );
